...
function playSound(soundPath) {
    ...
    if (platform === 'darwin') {
        (0, child_process_1.execFile)('afplay', [soundPath]);
    }
    else if (platform === 'win32') {
        (0, child_process_1.execFile)('powershell', ['-c', `(New-Object Media.SoundPlayer '${soundPath}').PlaySync()`]);
    }
    else {
        (0, child_process_1.execFile)('aplay', [soundPath], err => {
            if (err)
                (0, child_process_1.execFile)('paplay', [soundPath]);
        });
    }
}
class FaaaaahViewProvider {
    ...
    getHtml(webview) {
        const cfg = vscode.workspace.getConfiguration('faaaaah');
        const minNewErrors = cfg.get('minNewErrors', 1);
        const warningSoundEnabled = cfg.get('warningSoundEnabled', false);
        const minNewWarnings = cfg.get('minNewWarnings', 1);
        const soundFile = cfg.get('soundFile', '');
        const warningSoundFile = cfg.get('warningSoundFile', '');
...
FaaaaahViewProvider.viewId = 'faaaaah.settingsView';
function activate(context) {
    ...
    context.subscriptions.push(vscode.window.registerWebviewViewProvider(FaaaaahViewProvider.viewId, provider));
